...
void handle_hypercall(char *mem, uint32_t msg_buf_addr) {
  ...
  case IAI_WRITE: {
    ...
    IAI_LOG("WRITE", "guest_fd=%-2d len=%-4u -> ret=%-4d err=%d", req->args[0],
            req->data_len, req->ret, req->err);
    break;
  }
  case IAI_SOCKET: {
    ...
    IAI_LOG("SOCKET",
            "domain=%-2d type=%-2d proto=%-2d -> guest_fd=%-2d err=%d",
            req->args[0], req->args[1], req->args[2], req->ret, req->err);
    break;
  }
  case IAI_CONNECT: {
    int fd =
    ...
    IAI_LOG("CONNECT", "guest_fd=%-2d addrlen=%-2u -> ret=%-2d err=%d",
            req->args[0], req->data_len, req->ret, req->err);
    break;
  }
  case IAI_SEND: {
    int fd =
    ...
    IAI_LOG("SEND", "guest_fd=%-2d len=%-4u flags=%-2d -> ret=%-4d err=%d",
            req->args[0], req->data_len, req->args[1], req->ret, req->err);
    break;
  }
  case IAI_RECV: {
    int fd =
    ...
    IAI_LOG("RECV", "guest_fd=%-2d maxlen=%-4u flags=%-2d -> ret=%-4d err=%d",
            req->args[0], req->args[1], req->args[2], req->ret, req->err);
    break;
  }
  case IAI_CLOSE: {
    ...
    IAI_LOG("CLOSE", "guest_fd=%-2d -> ret=%-2d err=%d", req->args[0], req->ret,
            req->err);
    break;
  }
