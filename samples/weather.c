...
int main() {
    ...
    const char *path = "/v1/forecast?latitude=35.6895&longitude=139.6917&current_weather=true";
    char request[512];
    char buffer[1024];
    uint32_t ip_addr;

    /* 1. Resolve hostname */
    if (gethostbyname_r(hostname, &ip_addr) != 0) {
    ...
    while (*req_p1) request[req_len++] = *req_p1++;
    const char *req_path = path;
    while (*req_path) request[req_len++] = *req_path++;
    const char *req_p2 = " HTTP/1.1\r\nHost: ";
    while (*req_p2) request[req_len++] = *req_p2++;
    const char *req_host = hostname;
    while (*req_host) request[req_len++] = *req_host++;
    const char *req_p3 = "\r\nConnection: close\r\n\r\n";
    while (*req_p3) request[req_len++] = *req_p3++;
